...
graalvm)
    ...
    mvn -B ${INTEGRATION_TEST_ARGS} -ntp -Pnative -Penable-integration-tests test "-Dtest=com.google.cloud.spanner.adapter.it.**"
    RETURN_CODE=$?
    ;;
graalvm17)
    # Run Unit and Integration Tests with Native Image.
...
then
    ...
    ${KOKORO_GFILE_DIR}/linux_amd64/flakybot -repo=googleapis/java-spanner-cassandra
fi
